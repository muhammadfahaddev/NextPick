...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: match, error: matchError } = await supabase
      .from('matches')
      .select('id, team1_name, team2_name, match_datetime, status')
      .eq('id', match_id)
      .single();
    ...
    // Validate predicted_team is one of the two teams
    if (predicted_team !== match.team1_name && predicted_team !== match.team2_name) {
